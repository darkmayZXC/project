...
function ResultsComponent() {
  ...
  const scores: { theme: RiasecTheme; score: number }[] = riasecOrder.map(
    (theme) => ({
      theme,
      score: parseInt(searchParams.get(theme) || '0', 10),
    })
  );
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      pdf.save('Компас-Карьеры-Результаты.pdf');
    } catch (error) {
  ...
  const primaryTheme = topThree[0].theme;
  const suggestedCareers = careers[primaryTheme];

  const chartData = riasecOrder.map((theme) => ({
    theme: riasecData[theme].name,
    score: scores.find((s) => s.theme === theme)?.score || 0,
    fill: `var(--chart-${riasecOrder.indexOf(theme) + 1})`,
  }));

  const chartConfig = riasecOrder.reduce((acc, theme, index) => {
    acc[theme] = {
      label: riasecData[theme].name,
       ...
       <div ref={resultsRef} className="bg-background rounded-lg p-4 sm:p-8">
        <Card className="shadow-lg border-0">
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold">Ваши результаты</CardTitle>
            <CardDescription className="text-lg">
              Ваш код Холланда:{' '}
              <span className="font-bold text-primary">
                {topThree.map((s) => s.theme).join('')}
              </span>
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-8">
            ...
            <div className="space-y-6">
                {topThree.map((item, index) => {
                  const themeData = riasecData[item.theme];
                  return (
                    <Card key={item.theme}>
                      <CardHeader>
                        <div className="flex items-center gap-4">
                          <div className="bg-primary/10 p-3 rounded-lg w-fit">
                            <themeData.Icon className="h-8 w-8 text-primary" />
                          </div>
                          <div>
                            ...
                            <CardDescription>
                              {index === 0
                                ? 'Ваш основной тип личности'
                                : 'Дополнительный тип личности'}
                            </CardDescription>
                          </div>
                        </div>
                      </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  На основе вашего основного типа личности, вот несколько
